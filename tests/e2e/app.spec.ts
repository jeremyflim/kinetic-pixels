...
test('Monitor pins and reselects a live pixel until Escape or another control cancels it', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /Play|Pause/ }).hover()
  await expect(panel).toBeVisible()
  await expect(monitor).toHaveAttribute('aria-pressed', 'true')
  await canvas.click({ position: { x: 90, y: 70 } })
  await expect.poll(() => panel.locator('header b').textContent()).not.toBe(firstCoordinate)
  await expect(monitor).toHaveAttribute('aria-pressed', 'true')
